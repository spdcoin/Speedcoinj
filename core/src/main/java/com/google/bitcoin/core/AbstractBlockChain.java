 ...
 * 1440 blocks are examined and a new difficulty target is calculated from them.</p>
 */
public abstract class AbstractBlockChain {
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        if (params.getId().equals(NetworkParameters.ID_TESTNET)) {
            if (storedPrev.getHeight()+1 >= 5290) { DiffMode = 2; }
        }
        else {
            if (storedPrev.getHeight()+1 >= 2) { DiffMode = 2; }
        }
        ...
        //long elapsed = System.currentTimeMillis() - now;
        //log.info("Speedcoin checkDifficultyTransitions({}) is {} seconds", storedPrev.getHeight(), elapsed/1000);
    }
    private void checkDifficultyTransitions_V1(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        if (  (fUseFilter && (storedPrev.getHeight()+1) % nInterval != 0) ||
                (!fUseFilter && (storedPrev.getHeight()+1) % 1440 != 0))
        {
        ...
        if ( fUseFilter ) {
            ...
            for (; idx!=WINDOW && pitr != null && pprev != null; ++idx) {
                ...
                else if (pitr == null && lastHeight < 1440)
                {
...
//        if (bnNew > bnProofOfWorkLimit)
  //          bnNew = bnProofOfWorkLimit;
    ...
    private void verifyDifficulty(BigInteger calcDiff, Block nextBlock)
    {
        ...
        // temporary disable, work incorrectly with KGW 
...
*/
	...
	/* current difficulty formula, speedcoin - kimoto gravity well */
