...
/**
 * A tool for comparing the blocks which are accepted/rejected by bitcoind/speedcoinj
 * It is designed to run as a testnet-in-a-box network between a single bitcoind node and speedcoinj
...
public class BitcoindComparisonTool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof BlockAndValidity) {
                ...
                if (!chain.getChainHead().getHeader().getHash().equals(bitcoindChainHead)) {
                    differingBlocks++;
                    log.error("bitcoind and speedcoinj acceptance differs on block \"" + block.ruleName + "\"");
                }
        ...
        log.info("Done testing.\n" +
                "Blocks which were not handled the same between bitcoind/speedcoinj: " + differingBlocks + "\n" +
